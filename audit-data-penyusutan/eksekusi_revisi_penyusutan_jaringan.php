...
$query_data = "SELECT  aset_id,umurekonomis,MasaManfaat,kd_riwayat,TglPerubahan,NilaiPerolehan,NilaiPerolehan_Awal,(NilaiPerolehan-NilaiPerolehan_Awal)as np,AkumulasiPenyusutan,AkumulasiPenyusutan_Awal,(AkumulasiPenyusutan-AkumulasiPenyusutan_Awal)as akm,nilaibuku,NilaiBuku_Awal, PenyusutanPerTahun,PenyusutanPerTahun_Awal,bp_log,bp,(bp-bp_log)as selisih FROM `perhitungan_reklas_jaringan` where ((bp_log-bp)>2 or (bp_log-bp)<-2) and Kd_Riwayat in(50,51) group by aset_id ";
...
while ($row = $DBVAR->fetch_array ($result)) {
    ...
    echo "$info<br/>";
    logfile($info,"eksekusiBPK-jaringan/revisi-$Aset_ID.txt");
    ...
    while($row_history=$DBVAR->fetch_array ($result_history)){
        ...
        if($status_pertama==0){
            ...
            $PenyusutanPerTahun_log=$PenyusutanPerTahun;
                 ...
                 $query="INSERT INTO `log_jaringan`(`log_id`, `Aset_ID`, `kodeKelompok`,
              ...
              $query_master="update jaringan set NilaiBuku='$NilaiBuku',AkumulasiPenyusutan='$AkumulasiPenyusutan',
        ...
        logfile($hasil,"eksekusiBPK-jaringan/revisi-$Aset_ID.txt");
