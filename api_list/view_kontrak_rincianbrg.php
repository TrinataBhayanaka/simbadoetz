...
if (!empty($data)){
	foreach ($data as $key => $aRow)
	{
		...
		if($aRow['n_status'] != 1)
		{
			$view = "<a href=\"kontrak_barang.php?id={$aRow['id']}\" class=\"btn btn-info btn-small\"><i class=\"icon-plus icon-white\"></i>&nbsp;tambah</a>";
		} else 
		{
			$view = "<a href=\"kontrak_barang.php?id={$aRow['id']}\" class=\"btn btn-info btn-small\"><i class=\"fa fa-eye\"></i>&nbsp;View</a>";
		}
