...
$dataSESSION = $MUTASI->retrieve_daftar_aset_penetapan($dataParam); 
//pr($dataSESSION);
...
$sQuery = "
    ...
    FROM   $sTable INNER JOIN mutasiaset as p 
    ON p.Aset_ID = a.Aset_ID WHERE p.Mutasi_ID  in ($dataParam[id])";
...
$data_post=$MUTASI->apl_userasetlistHPS("DELASMTS");

$POST=$MUTASI->apl_userasetlistHPS_filter($data_post);
...
foreach ($data as $key => $value)
            {
              ...
              $SelectKIB=$MUTASI->SelectKIB($value[Aset_ID],$value[TipeAset]);
              //pr($SelectKIB);
              if($value[kondisi]==2){
              ...
              $TglPerolehan=$TglPerolehanTmp[2]."/".$TglPerolehanTmp[1]."/".$TglPerolehanTmp[0];
                ...
                //get status
                $status=$MUTASI->getStatusMutasi($dataParam['id']);
                //pr($status);
                if($status['0']['FixMutasi']==0){              
