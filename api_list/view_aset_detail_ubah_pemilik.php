...
$aColumns = array('a.Aset_ID','a.kodeKelompok','k.Uraian','a.Tahun','a.kodeSatker',
				 'a.StatusValidasi','a.Status_Validasi_Barang','a.NilaiPerolehan','a.noRegister','a.TipeAset','a.kodeLokasi');
$test = count($aColumns);
...
$status = "a.StatusValidasi = 1 AND a.Status_Validasi_Barang = 1 AND kodeLokasi LIKE '$pemilik%' AND";
$condtn = "a.Tahun = '$Tahun' AND a.kodeSatker='$kodeSatker' AND ";
...
if (!empty($data)){
	foreach ($data as $key => $aRow)
	{
		...
		$lok = explode(".",$aRow['kodeLokasi']);
		if($lok[0] == "0")
		{
			$lok = "0 Pemerintah Pusat";
		} elseif ($lok[0] == "11") {
			$lok = "11 Pemerintah Provinsi";
		} elseif ($lok[0] == "12") {
			$lok = "12 Pemerintah Kabupaten/Kota";
		} elseif ($lok[0] == "13") {
			$lok = "13 Pemerintah Provinsi Lain";
		} elseif ($lok[0] == "14") {
			$lok = "14 Pemerintah Kabupaten/Kota Lain";
		} elseif ($lok[0] == "15") {
			$lok = "15 Non Pemerintah";
		}
		//cek ruangan 
		if($kodeRuangan != ''){
