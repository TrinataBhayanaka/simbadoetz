 ...
 */
 ...
 // pr($_GET);
 // exit;
$aColumns = array('ua.Penyusutan_ID','ua.Aset_ID','a.kodeKelompok','a.noRegister','a.NilaiPerolehan','a.Tahun','a.Info','a.AkumulasiPenyusutan','a.PenyusutanPerTaun');
// $test = count($aColumns);
  
// echo $aColumns; 
/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "ua.Penyusutan_ID";

/* DB table to use */
$sTable = "penyusutanaset as ua";
$sTable_inner_join_aset = "aset as a";
$cond_aset ="a.Aset_ID = ua.Aset_ID";

//variabel ajax
//parameter sql
$id_usulanPenetapan = $_GET['Penetapan_ID'];
...
$sWhere=" WHERE ua.Penyusutan_ID in ({$id_usulanPenetapan}) ";


if (isset($_GET['sSearch']) && $_GET['sSearch'] != "") {
     $sWhere = "WHERE ua.Penyusutan_ID in ({$id_usulanPenetapan}) AND (";
...
if (!empty($rResultGetDataApluserlist)){
	foreach ($rResultGetDataApluserlist as $aRow)
	{
		// pr($aRow);
		//DAFTAR FIELD
