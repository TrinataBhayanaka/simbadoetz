...
$dataSESSION = $RETRIEVE_PEROLEHAN->get_tmpData($dataParam,'tmp_mesin'); 
//pr($dataSESSION);
//exit;
...
$data_post=$PENGHAPUSAN->apl_userasetlistHPS("XLSIMPB");
...
if($POST){
  ...
  $newlist = array();
      ...
      foreach ($dataSESSION as $keySESSION => $valueSESSION) {
        ...
        if(!in_array($valueSESSION['tmp_Mesin_ID'], $newlist)){
          // echo "stringnot";
          $data[]=$valueSESSION;
...
foreach ($data as $key => $value)
               ...
               $checkbox="<input {$value['style']} type=\"{$value['other']}\" {$value['checked']} id=\"check_{$no}\" class=\"icheck-input\" name=\"aset[]\" 
                      value=\"{$value['tmp_Mesin_ID']}|{$value['Jumlah']}|{$value['NilaiPerolehan']}\" onchange=\"return AreAnyCheckboxesChecked();\">";
               $row[]=$no;
               $row[]=$checkbox;
