...
$dataSESSION = $RETRIEVE_PEROLEHAN->get_tmpData($dataParam,'tmp_bangunan'); 
//pr($dataSESSION);
//exit;
...
$data_post=$PENGHAPUSAN->apl_userasetlistHPS("XLSIMPC");
...
if($POST){
  ...
  $newlist = array();
      ...
      foreach ($dataSESSION as $keySESSION => $valueSESSION) {
        ...
        if(!in_array($valueSESSION['temp_Bangunan_ID'], $newlist)){
          // echo "stringnot";
          $data[]=$valueSESSION;
...
foreach ($data as $key => $value)
               ...
               $checkbox="<input {$value['style']} type=\"{$value['other']}\" {$value['checked']} id=\"check_{$no}\" class=\"icheck-input\" name=\"aset[]\" 
                      value=\"{$value['temp_Bangunan_ID']}|{$value['NilaiTotal']}\" onchange=\"return AreAnyCheckboxesChecked();\">";
               $row[]=$no;
               $row[]=$checkbox;
