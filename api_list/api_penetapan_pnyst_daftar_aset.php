 ...
 */
 ...
 // pr($_GET);
 // exit;
$aColumns = array('ua.Usulan_ID','ua.Aset_ID','ua.Jenis_Usulan','a.kodeKelompok','a.noRegister','a.NilaiPerolehan','a.Tahun','a.Info','a.AkumulasiPenyusutan','a.PenyusutanPerTaun');
// $test = count($aColumns);
...
//variabel ajax
//parameter sql
$id_usulanPenetapan = $_GET['id_usulanPenetapan'];
...
if (!empty($data)){
	foreach ($data as $key => $aRow)
	{
		...
		if($AkumulasiPenyusutan == '' || empty($AkumulasiPenyusutan) || $PenyusutanPerTaun == '' || empty($PenyusutanPerTaun)){
			$NilaiBuku = '';
		}else{
