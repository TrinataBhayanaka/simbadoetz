...
echo "-- Akhir Untuk Aset Mesin==TOTAL=$no\n\n";
/**
 * Akhir Sinkronisasi Aset untuk Mesin
 */
...
$query="select Aset_ID,NilaiBuku,AkumulasiPenyusutan,PenyusutanPerTahun,MasaManfaat,UmurEkonomis,TahunPenyusutan,kondisi,status_validasi_barang
          from $db_auditied_2016.mesin where kodeKelompok like '02%' and (TglPerolehan!='0000-00-00' and TglPerolehan is not NULL)
          and (TglPembukuan!='0000-00-00' and TglPembukuan is not NULL) 
          ";
$result=mysql_query($query) or die(mysql_error());
echo "-- MESIN(02)\n\n";
...
while($row=mysql_fetch_array($result)){
    ...
    $query="update mesin set  NilaiBuku='$NilaBuku', AkumulasiPenyusutan='$AkumulasiPenyusutan',
            PenyusutanPerTahun='$PenyusutanPerTahun',MasaManfaat='$MasaManfaat',
            UmurEkonomis='$UmurEkonomis',TahunPenyusutan='$TahunPenyusutan' where Aset_ID='$Aset_ID'; ";
    $no++;
    echo "$query\n";
}
echo "-- AKHIR UNTUK MESIN(02)==TOTAL==$no\n\n";
/**
 * Akhir Sync untuk mesin
 */
