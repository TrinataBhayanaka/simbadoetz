    <?php
        ...
        $nama_aset=$_POST['peman_penet_bmd_filt_add_nmaset'];
        $nomor_kontrak=$_POST['peman_penet_bmd_filt_add_nokontrak'];
        $satker=$_POST['skpd_id'];
        $submit=$_POST['tampil_idle_add'];
        //open_connection();
        
        if ($nama_aset!=""){
            $query_nama_aset="SELECT a.Aset_ID FROM Aset AS a LEFT JOIN PenggunaanAset AS b ON b.Aset_ID=a.Aset_ID
		...
		$parameter_sql_report="WHERE ".$parameter_sql_report." AND ";
            ...
            $query_apl = "SELECT aset_list FROM apl_userasetlist WHERE aset_action = 'Menganggur' AND UserSes = '$_SESSION[ses_uid]'";
            //print_r($query_apl);
        ...
        if (isset($submit)){
                if ($nama_aset=="" && $nomor_kontrak=="" && $satker==""){
    ?>
                    ...
                    <div id="frame_tengah1">
                        <div id="frame_gudang">
                            ...
                            <div id="bottomright">
                                ...
                                <form name="form" method="POST" action="<?php echo "$url_rewrite/module/pemanfaatan/"; ?>pemanfaatan_penetapan_idle_eksekusi_data.php">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td colspan=3><p style="float:left;"><u onmouseover="this.style.backgroundColor='orange'" onmouseout="this.style.backgroundColor=''" id="pilihHalamanIni">Pilih halaman ini</u>&nbsp;&nbsp; || 
                                                                        ...
                                                                        <p style="float:right;"><input type="submit" name="idle_penet" value="Lanjutkan" id="submit" disabled/></p>
                                            </td>
                                            <td></td>
                                            <td></td>
                                        </tr>
                                        ...
                                        <?php
                                                ...
                                                $query="SELECT Aset_ID FROM PenggunaanAset $_SESSION[parameter_sql]  Status=1 
                                                                                    AND StatusMenganggur=0 AND StatusMutasi=0 
                                                                                    ORDER BY Aset_ID asc limit $offset, $jmlperhalaman";
                                                $query_total_record="SELECT COUNT(*) FROM PenggunaanAset $_SESSION[parameter_sql]  Status=1 
                                                                                    AND StatusMenganggur=0 AND StatusMutasi=0";
                                        ...
                                        <tr>
                                            ...
                                            <td align="center" style="border: 1px solid #004933; height:100px; color: black; font-weight: bold;">
                                                 <input type="checkbox" class="checkbox" onchange="enable()" name="Menganggur" value="<?php echo $value->Aset_ID;?>" <?php for ($j = 0; $j <= count($explode); $j++){if ($explode[$j]==$value->Aset_ID) echo 'checked';}?>/>
                                            </td>
                                                            ...
                                                            <table width="50%">
                                                                <tr>
                                                                    ...
                                                                    <td><?php
                                                                                /*
                                                                               $idaset=$row['Aset_ID'];
                                                                               $query_fix_kontrak_aset1="SELECT * FROM KontrakAset where Aset_ID='$idaset'";
