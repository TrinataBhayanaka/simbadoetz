...
include"$path/menu.php";
	...
	$par_data_table ="bup_tahun={$bup_tahun}&jenisaset={$jenisaset}&kodeSatker={$kodeSatker}&kodepemilik={$kodepemilik}&kodeKelompok={$kodeKelompok}&usulanID={$usulanID}";
	?>
	<script>
		  ...
		  if ($("#Form2 input:checkbox:checked").length > 0)
			{
			    ...
			    updDataCheckbox('RVWUSMTSKPTLS');
			    countCheckbox('RVWUSMTSKPTLS');
			}
			else
			{
          ...
          $('#usulan_mts').dataTable(
                   {
                    ...
                    "bprocessing": true,
                    "bServerSide": true,
                    "sAjaxSource": "<?=$url_rewrite?>/api_list/mutasiKptls/api_aset_usulan_mutasi.php?<?php echo $par_data_table?>"
               }
                  );
      });
    </script>
			<div id="demo">
			<form method="POST" ID="Form2" onsubmit="return confirmValidate()"  action="<?php echo"$url_rewrite"?>/module/mutasiKptls/usulan_aset_proses.php"> 
				...
				<thead>
					...
					<tr>
						...
						<td colspan="3" align="right">
							<a href="<?php echo"$url_rewrite"?>/module/mutasiKptls/filter_tambah_aset_usulan.php?usulanID=<?php echo $_POST[usulanID] ?>" class="btn">Kembali Ke Pencarian</a>
						</td>
					</tr>
					<tr>
						<th>No</th>
