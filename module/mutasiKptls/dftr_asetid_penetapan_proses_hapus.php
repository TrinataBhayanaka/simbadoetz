...
if($_POST['cekAll'] == 1){
    $id = $_POST['Mutasi_ID'];
    $log = "hapus_aset_penetapan_mtskptls_".$id;  
    
    $status=exec("php hapus_aset_penetapan_mts_helper_all.php $id > ../../log/$log.txt 2>&1 &");
    echo "<meta http-equiv=\"Refresh\" content=\"0; url={$url_rewrite}/module/mutasiKptls/dftr_review_edit_aset_penetapan.php?id=$id\">";    
    exit;
}else{
    $id = $_POST['Mutasi_ID'];
    $log = "hapus_aset_penetapan_mtskptls_".$id;

    $apl_userasetlistHPS = $MUTASI->apl_userasetlistHPS("DELASMTSKPTLS");
