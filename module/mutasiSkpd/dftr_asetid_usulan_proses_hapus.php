...
if($_POST['cekAll'] == 1){
    $id = $_POST['usulanID'];
    $log = "hapus_aset_usulan_mts_".$id;

    $status=exec("php hapus_aset_usulan_mts_helper_all.php $id > ../../log/$log.txt 2>&1 &");
    echo "<meta http-equiv=\"Refresh\" content=\"0; url={$url_rewrite}/module/mutasiSkpd/list_usulan.php?id=$id\">";    
    exit;
}else{
    $id = $_POST['usulanID'];
    $log = "hapus_aset_usulan_mts_".$id;

    $apl_userasetlistHPS = $MUTASI->apl_userasetlistHPS("DELUSMTS");
