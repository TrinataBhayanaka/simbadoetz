...
?>
    ...
    //pr($Penghapusan_ID);
    $data_post=$MUTASI->apl_userasetlistHPS("RVWPTUSMTS");
		  ...
		  if ($("#Form2 input:checkbox:checked").length > 0)
			{
			    ...
			    updDataCheckbox('PTUSMTS');
			    countCheckbox('PTUSMTS');
			}
			else
			{
          	...
          	$('#rvw_aset_usulan').dataTable(
                   {
                    ...
                    "bProcessing": true,
                    "bServerSide": true,
                    "sAjaxSource": "<?=$url_rewrite?>/api_list/api_review_edit_aset_penetapan_mutasi.php?<?php echo $par_data_table?>"
               });
      	});
		</script>
	<section id="main">
		...
		<section class="formLegend">
			<form name="form" method="POST" ID="Form2" onsubmit="return confirmValidate()" action="<?php echo "$url_rewrite/module/mutasiSkpd/"; ?>penetapan_tambah_data_proses.php">
