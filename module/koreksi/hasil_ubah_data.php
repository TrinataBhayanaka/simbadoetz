...
<script>
	function getminmax(item)
	{
		...
		if(parseInt($(item).val()) > max || $(item).val() == 0 )
		{
			...
			$("#url_"+idnumber[1]).attr('href','ubah_data.php?kdkel='+others[0]+'&kdlok='+others[1]+'&reg='+$(item).val()+'&tbl='+others[2]);
		}	
	}
</script>
          <section id="main">
				...
				?>
						<tr class="gradeA">
							...
							<td class="text-center">
								<a href="ubah_data.php?kdkel=<?=$value['kodeKelompok']?>&kdlok=<?=$value['kodeLokasi']?>&reg=<?=$value['min']?>&tbl=<?=$_POST['tipeAset']?>" id="url_<?=$i?>" class="btn btn-success btn-small" ><i class="fa fa-edit"></i>&nbsp;Koreksi</a>
							</td>
						</tr>
				<?php
						$i++;
