...
$query = "SELECT * FROM tmp_asetlain" or die("Error in the consult.." . mysqli_error($link));

$result_masuk = $link->query($query); 
$cleardata=array();
...
$counter = 0;
$totaldata = 0;
while($row_hasil = mysqli_fetch_assoc($result_masuk)) {
	$counter++;
	//$tmp = explode("|", $val);
        $temp_AsetLain_ID=$row_hasil["temp_AsetLain_ID"];
        echo "$temp_AsetLain_ID\n";
	$sql = "SELECT * FROM tmp_asetlain WHERE temp_AsetLain_ID = '{$temp_AsetLain_ID}'" or die("Error in the consult.." . mysqli_error($link));
	...
	echo "ID Data di proses:".$temp_AsetLain_ID."\n";
...
$sql = "UPDATE log_import SET totalPerolehan = '{$sum['sumnilai']}', status = 1 WHERE noKontrak = 'kpad-2016'";
$exec = $link->query($sql);
