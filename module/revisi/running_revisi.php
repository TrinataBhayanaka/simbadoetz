  ...
  if($status_running==0){
          switch ($kelompok) {
               case "Peralatan dan Mesin (B)":
                    $query="update  penyusutan_tahun_berjalan  set status_revisi=1 where id=$id";
                    $DBVAR->query($query) or die($DBVAR->error());
                    $status=exec("php running_revisi_server.php B $Tahun $kodeSatker $id > $path/log/$Tahun-revisi-log-B-$kodeSatker.txt &");
					header('Location: revisi.php');
					break;
			case "Gedung dan Bangunan (C)":
                   $query="update  penyusutan_tahun_berjalan  set status_revisi=1 where id=$id";
                  $DBVAR->query($query) or die($DBVAR->error());
                   $status=   exec("php running_revisi_server.php C $Tahun $kodeSatker $id > $path/log/$Tahun-revisi-log-C-$kodeSatker.txt &");
                   header('Location: revisi.php');
				   break;
              ...
              case "Jalan, Irigrasi, dan Jaringan (D)":
                   $query="update  penyusutan_tahun_berjalan  set status_revisi=1 where id=$id";
                   $DBVAR->query($query) or die($DBVAR->error());
                   $status=   exec("php running_revisi_server.php D $Tahun $kodeSatker $id > $path/log/$Tahun-revisi-log-D-$kodeSatker.txt &");
                   header('Location: revisi.php');
				   break;
			}
         // echo $status;
