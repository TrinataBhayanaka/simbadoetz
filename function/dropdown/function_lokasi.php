...
function js_checkboxlokasi($alamat_simpul,$alamat_search,$parsing,$save_element,$tbody_name,$prefix){
  ...
  echo "
		  ...
		  function SelectAllChild_$tbody_name(id) {
                   ...
                   m=document.getElementById(b).checked;
                    if (m!=1)
                   {
                         set_parent_lokasi(b,m);
                    }
                    
                   setCheckBox_lokasi__$tbody_name(\"$tbody_name\", \"input\", b, m);
                   show_result_lokasi__$tbody_name(\"$tbody_name\", \"input\", \"$prefix\", \"$parsing\");


              }
		  </script>
               <script type=\"text/javascript\">
               function show_result_lokasi__$tbody_name(container, selectorTag, prefix,element_update) {
         ...
         <script>
              function setCheckBox_lokasi__$tbody_name(container, selectorTag, prefix,nilai,element_update) {
               var items = [];
               var hasil='';
               var myPosts = document.getElementById(container).getElementsByTagName(selectorTag);
               var c=1;
...
function checkboxlokasi($style_div,$save_element,$tbody_name,$prefix){
         ...
         while($row = mysql_fetch_object($result))
          {
               ...
               echo "<tr id='$kode_for_js'>";
                    echo "<td> &nbsp;<input type=\"checkbox\" name=\"$tbody_name"."[]\" id=\"$prefix"."_$kode\" value=\"$lokasi_id\" onclick=\"SelectAllChild_$tbody_name(this);\"></td>";
