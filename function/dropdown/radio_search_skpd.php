<?php
  ...
  $sql=mysql_query("SELECT * FROM Satker WHERE KodeSektor is not NULL and KodeSatker is NULL and KodeUnit is NULL and Gudang is NULL and NGO='0' order by KodeSektor asc");
  }
...
$sql="SELECT * FROM Satker WHERE KodeSektor is not NULL and KodeSatker is NULL and KodeUnit is NULL and Gudang is NULL and NGO='0' order by KodeSektor asc";
...
while($row = mysql_fetch_object($result))
{
	 ...
	 $sql_satker=mysql_query("select * from Satker where KodeSektor='$kodesektor_tmp[$i]' and KodeSatker='$kodesatker_tmp[$i]' and KodeUnit is null and Gudang is null ORDER by KodeSatker ASC");
...
{	
	 ...
	 $sql_unit=mysql_query("select * from Satker where KodeSektor='$kodesektor_tmp[$j]' and KodeSatker='$kodesatker_tmp[$j]' and KodeUnit='$kodeunit_tmp[$j]' and Gudang is null ORDER by KodeSatker,KodeUnit ASC");
                  ...
                  //cari kode upb
                         ...
                         for ($k = 0; $k < $total; $k++) {
                              if ($kode_sektor == $kodesektor_tmp[$k] && $distinct_upb != $kodeupb_tmp[$k]) {
                                   $sql_upb = mysql_query("select * from Satker where KodeSektor='$kodesektor_tmp[$k]' and KodeSatker='$kodesatker_tmp[$k]' and KodeUnit='$kodeunit_tmp[$k]' and Gudang='$kodeupb_tmp[$k]' ORDER by KodeSatker,Gudang ASC");
                                   while ($row = mysql_fetch_object($sql_upb)) {
                                        $skpd_id = $row->Satker_ID;
                                        $kode = $row->KodeSatker . "." . $row->KodeUnit . "." . $row->Gudang;
                                        $uraian = $row->NamaSatker;
