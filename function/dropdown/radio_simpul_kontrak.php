...
if($tahun!="")
{
     $query_golongan=" TglKontrak like '$tahun%'";
}
...
if($kode!=""&&$paramater_sql!=""){
     $query="Select * from Kontrak where $paramater_sql order by TglKontrak asc";
//      echo "$query  <br/>";
...
$no=1;
     while($row=  mysql_fetch_object($result)){
          ...
          $kode_for_js="$prefix-kontrak_row_1|$kode|$level";
          //$sign='1';
        //initinya
		
		           echo "$kode_for_js]$spasi]";
                    echo "$kode $no]";
                    echo "add_kontrak$prefix('$kode_for_js','$prefix"."_$kode')]$uraian]$parent_node]$kontrak_id]$level]$prefix"."_$kontrak_id]$last+";
          //intinya   
		  $no++;         
