  ...
  else
  {
	...
	echo "<table border='1'>
		<thead>
			...
			<th rowspan='3'>MERK / TIPE</th>
			<th rowspan='3'>UKURAN /<br>CC</th>
	...
	echo "<tr>";
		...
		list($nm_brg,$kd_brg,$noreg,$kd_lokasi,$merk,$ukuran,$bahan,$thn_pembelian,
		$pabrik,$rangka,$mesin,$polisi,$bpkb,$asalusul,$harga,$skpd_id,$ket,$user) = explode("|", $row[$k]);
		//cari kelompok_id
	  ...
	  // setelah data dibaca, sisipkan ke dalam tabel rkb
		$query = "INSERT INTO Aset (Aset_ID,NamaAset,Kelompok_ID,NomorReg,Lokasi_ID,Tahun,AsalUsul,NilaiPerolehan,OrigSatker_ID,Pemilik,TipeAset,Info,UserNm,NotUse,CaraPerolehan,Kuantitas) 
		VALUES ('','$nm_brg', '$kelompok_id', '$noreg', '$kd_lokasi', '$thn_pembelian', '$asalusul', '$harga', '$skpd_id', '$pemilik','B','$ket','$user','0','0','1')";
		$hasil = mysql_query($query) or die(mysql_query());
		
		//cari aset_id
		$sql="SELECT Aset_ID FROM Aset 
				WHERE 
					Kelompok_ID='$kelompok_id' AND NamaAset='$nm_brg' AND NomorReg='$noreg' AND Tahun='$thn_pembelian'
		...
		$query2 = "INSERT INTO Mesin (Mesin_ID,Aset_ID,Merk,Ukuran,Material,NoSeri,NoRangka,NoMesin,NoSTNK,NoBPKB) 
		VALUES ('','$aset_id','$merk','$ukuran','$bahan','$pabrik','$rangka','$mesin','$polisi','$bpkb')";
	...
	$status="Gagal di Upload";
	  ...
	  echo "<td style='text-align:center'>$j</td><td>$nm_brg</td><td>$kd_brg</td><td>$noreg</td><td>$kd_lokasi</td><td>$merk</td><td>$ukuran</td><td>$bahan</td><td>$thn_pembelian</td><td>
		$pabrik</td><td>$rangka</td><td>$mesin</td><td>$polisi</td><td>$bpkb</td><td>$asalusul</td><td>$harga</td><td>$ket</td><td>$status</td></tr>";
    }
  }
...
?>
<br>
<a href="<?php echo $url_rewrite ?>/import/kibb.php" style="font:14px Arial;color:#0066FF;text-decoration:underline;">&nbsp&nbspImport Data Lagi&nbsp&nbsp</a>						
