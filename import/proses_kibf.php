...
echo "<form action='proses_kibf_hasil.php' method='post' name='sheet'>";
...
for ($i=15; $i<=$baris; $i++)
{
  ...
  $bangunan = $data->val($i, 5);
  // membaca data lokasi (kolom ke-3)
  $tingkat = $data->val($i, 6);
  ...
  $tgl_dokumen = $data->val($i, 11);
  // membaca data harga (kolom ke-8)
  $no_dokumen = $data->val($i, 12);
  // membaca data total harga (kolom ke-9)
  $tgl_mutasi = $data->val($i, 13);
  // membaca data total harga (kolom ke-9)
  $status_tanah = $data->val($i, 14);
  $kd_tanah = $data->val($i, 15);
  $tahun_pengadaan = $data->val($i, 16);
  $asalusul = $data->val($i, 17);
  $harga = $data->val($i, 18);
  $ket = $data->val($i, 19);
  
  //echo "$tahun_pembelian";
  //echo "$nilai_kontrak";
...
echo "
		...
		<input type='checkbox' $require value='$nm_brg|$kd_brg|$noreg|$lokasi_id|$bangunan|$tingkat|$beton|$luas|
		$alamat|$rtrw|$tgl_dokumen|$no_dokumen|$tgl_mutasi|$status_tanah|$kd_tanah|$tahun_pengadaan|$asalusul|$harga_unit|$skpd_id|$ket|$user' />
		</td><td>$nm_brg</td><td $require_brg>$kd_brg</td><td>$noreg</td><td>$lokasi_id</td><td>$bangunan</td><td>$tingkat</td><td>$beton</td><td>$luas</td><td>
		$alamat</td><td>$rtrw</td><td>$tgl_dokumen</td><td>$no_dokumen</td><td>$tgl_mutasi</td><td>$status_tanah</td><td>$kd_tanah</td><td $require_thn>$tahun_pengadaan</td><td>$asalusul</td><td $require_harga>$harga_unit</td><td>$ket</td>
		</tr>		
	";
...
echo "
		...
		<td width='50' style='text-align:center'>
		<input type='checkbox' $require value='$nm_brg|$kd_brg|$noreg|$lokasi_id|$bangunan|$tingkat|$beton|$luas|
		$alamat|$rtrw|$tgl_dokumen|$no_dokumen|$tgl_mutasi|$status_tanah|$kd_tanah|$tahun_pengadaan|$asalusul|$harga|$skpd_id|$ket|$user' />
		</td><td>$nm_brg</td><td $require_brg>$kd_brg</td><td>$noreg</td><td>$lokasi_id</td><td>$bangunan</td><td>$tingkat</td><td>$beton</td><td>$luas</td><td>
		$alamat</td><td>$rtrw</td><td>$tgl_dokumen</td><td>$no_dokumen</td><td>$tgl_mutasi</td><td>$status_tanah</td><td>$kd_tanah</td><td $require_thn>$tahun_pengadaan</td><td>$asalusul</td><td $require_harga>$harga</td><td>$ket</td>
		</tr>		
	";
	}
  }
...
&nbsp<a href='kibf.php'><input type='button' value='Cancel' style='width:100px'/></a></div>
</form>";
