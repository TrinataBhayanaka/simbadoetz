...
echo "<form action='proses_kibd_hasil.php' method='post' name='sheet'>";
...
for ($i=13; $i<=$baris; $i++)
{
  ...
  // membaca data total harga (kolom ke-9)
  $no_tanah = $data->val($i, 15);
  $asalusul = $data->val($i, 16);
  $harga = $data->val($i, 17);
  $kondisi = $data->val($i, 18);
  $ket = $data->val($i, 19);
...
echo "
		...
		<input type='checkbox' $require value='$nm_brg|$kd_brg|$noreg|$lokasi_id|$konstruksi|$tahun_pengadaan|$panjang|$lebar|$luas|$alamat|$rtrw|$tgl_dokumen|$no_dokumen|
		$status_tanah|$no_tanah|$asalusul|$harga_unit|$kondisi|$skpd_id|$ket|$user' />
		</td><td>$nm_brg</td><td $require_brg>$kd_brg</td><td>$noreg</td><td>$lokasi_id</td><td>$konstruksi</td><td $require_thn>$tahun_pengadaan</td><td>$panjang</td><td>$lebar</td><td>$luas</td><td>$alamat</td><td>$rtrw</td><td>$tgl_dokumen</td>
		<td>$no_dokumen</td><td>$status_tanah</td><td>$no_tanah</td><td>$asalusul</td><td $require_harga>$harga_unit</td><td>$kondisi</td><td>$ket</td>
		</tr>		
	";
...
echo "
		...
		<td width='50' style='text-align:center'>
		<input type='checkbox' $require value='$nm_brg|$kd_brg|$noreg|$lokasi_id|$konstruksi|$tahun_pengadaan|$panjang|$lebar|$luas|$alamat|$rtrw|$tgl_dokumen|$no_dokumen|
		$status_tanah|$no_tanah|$asalusul|$harga|$kondisi|$skpd_id|$ket|$user' />
		</td><td>$nm_brg</td><td $require_brg>$kd_brg</td><td>$noreg</td><td>$lokasi_id</td><td>$konstruksi</td><td $require_thn>$tahun_pengadaan</td><td>$panjang</td><td>$lebar</td><td>$luas</td><td>$alamat</td><td>$rtrw</td><td>$tgl_dokumen</td>
		<td>$no_dokumen</td><td>$status_tanah</td><td>$no_tanah</td><td>$asalusul</td><td $require_harga>$harga</td><td>$kondisi</td><td>$ket</td>
		</tr>		
	";
	}
  }
...
&nbsp<a href='kibd.php'><input type='button' value='Cancel' style='width:100px'/></a></div>
</form>";
