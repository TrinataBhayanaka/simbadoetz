...
$query_aset_penghapusan="select Aset_ID,group_concat(kd_riwayat) as riwayat from log_jaringan where tglperubahan >='2015-01-01' group by Aset_ID having riwayat  like '7,49%' ";
...
$query_aset_transfer="select Aset_ID from log_jaringan where tglperubahan >='2015-01-01' group by Aset_ID having group_concat(kd_riwayat) like '3,49%' ";

$aset_penghapusan_pemindahtangan="201072,201073,201176,201293,201702,201732";
$query_aset_penghapusan_pemindahtangan="select Aset_ID from log_jaringan where tglperubahan >='2015-01-01' group by Aset_ID having group_concat(kd_riwayat) like '26,49%' ";

$aset_ubah_data_biasa="201785,201806";
$query_aset_ubah_data_biasa="select Aset_ID from log_jaringan where tglperubahan >='2015-01-01' group by Aset_ID having group_concat(kd_riwayat) like '18,49%' ";
...
while($row=mysql_fetch_array(($result))){
	$Aset_ID=$row['Aset_ID'];
	geser_log($link,$database,"log_jaringan",$Aset_ID);
}
...
function geser_log($link,$database,$tablename,$Aset_ID){
	$query="select * from log_jaringan where tglperubahan >='2015-01-01'  and 
			aset_id in ($Aset_ID)";
	...
	while($row=mysql_fetch_array($result)){
		...
		if($status_geser!=0){
			if($kd_riwayat ==50){
				...
				$update_aset="update log_jaringan set log_id=$tmp_logid_lama where log_id=$log_id";
				...
				$update_aset="update log_jaringan set log_id=$tmp_logid_lama where log_id=$tmp_logid_baru";
				...
				$update_aset="update log_jaringan set log_id=$tmp_logid_baru where log_id=$tmp_logid_lama";
