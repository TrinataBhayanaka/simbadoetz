...
$query = "SELECT log_id,Aset_ID,NilaiPerolehan,NilaiPerolehan_Awal,(NilaiPerolehan-NilaiPerolehan_awal) AS kapitalisasi,
          NilaiBuku,NilaiBuku_Awal FROM log_bangunan WHERE Kd_Riwayat IN (2)        
          AND kodesatker LIKE '05%' AND Tglperubahan >'2015-12-31' ";
$ExeQuery = $DBVAR->query ($query) or die($DBVAR->error ());

echo "/*Data Perbaikan Log Bangunan untuk perhitungan penyusutan 2016 -- kodesatker DPU (05):*/\n";
$i=1;
while ($Data = $DBVAR->fetch_array ($ExeQuery)) {
    ...
    $query_update="update log_bangunan set NilaiBuku='$NilaiBukuKapitalisasi',NilaiBuku_Awal='$NilaiBuku' where log_id='$log_id'; ";
    echo "$query_update\n";
    //$result_update=$DBVAR->fetch_array ($query_update);

    $query_update="update bangunan set NilaiBuku='$NilaiBukuKapitalisasi'  where Aset_ID='$Aset_ID'; ";
    echo "$query_update\n";
    //$result_update=$DBVAR->fetch_array ($query_update);
