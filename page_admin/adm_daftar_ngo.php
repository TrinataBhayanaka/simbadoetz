...
if (isset($_POST['Simpan']))
{
	...
	if ($_POST['opt_list'] == 'opt_bidang')
	{
		...
		$query = "INSERT INTO Satker VALUES (NULL, NULL, ".$KodeSektor.", NULL, ".$NamaSatker.", NULL,
				1, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, NULL, '$tmp_kode', NULL, 0)";
...
if (isset($_POST['Hapus']))
{
	...
	$sql = "SELECT Tmp_KodeSatker FROM Satker WHERE Satker_ID = '$Satker_ID' AND NGO = 1";
	//print_r($sql);
	...
	$query = "DELETE FROM Satker WHERE NGO IS TRUE AND Tmp_KodeSatker LIKE '$tmp_kode%'";
	//print_r($query);
	$result = $DBVAR->query($query) or die ($DBVAR->error());
	if ($result)
	{
...
if (isset($_POST['Update']))
{
	...
	if ($_POST['pr']!=='')
	{
		...
		{
			$Satker_ID = $_POST['sp'];
			$dataSatker = "$_POST[KodeSektor].$_POST[KodeSatker]";
			$tmp_kode = "$_POST[KodeSektor].$_POST[KodeSatker]";
			$query = "UPDATE Satker SET
							KodeSektor =".$KodeSektor.", KodeSatker ='$dataSatker',
							NamaSatker =".$NamaSatker.", Tmp_KodeSatker = '$tmp_kode'
	...
	<table width="100%" align="center" cellpadding="0" cellspacing="5"
		...
		<tr>
			...
			while ($data = $DBVAR->fetch_array($result))
			{
				...
				<div
					class="<?php if (isset($_GET['pr'])){if (($_GET['pr']==$data['Satker_ID']) and !isset($_GET['sp'])) echo 'datalist_inlist_selected';} ?>">
				<a class="datalist_inlist"
					href="?page=5&pr=<?php echo $data['Satker_ID']?>&a=v"><?php echo "<span style='font-size:12px'>BID $data[KodeSektor] $data[NamaSatker]</span>"; ?></a>
				</div>
				...
				if (isset($_GET['pr']))
				{
					if ($_GET['pr'] == $data['Satker_ID'])
					{
						...
						while ($dataSubParent = $DBVAR->fetch_array($rSubParent))
						{
							...
							<div style="margin-left: 15px; padding-left: 14px;">&raquo; <?php echo "<span style='font-size:12px'>$dataSubParent[KodeSatker] - $dataSubParent[NamaSatker]</span>"?></div>
							</a>
							</div>
								...
								if (isset($_GET['sp']))
								{
									if ($_GET['sp'] == $dataSubParent['Satker_ID'])
									{
										...
										while ($dataSubSubParent = $DBVAR->fetch_array($rSubSubParent))
										{
											...
											$old_subNGOID = explode('.',$dataSubSubParent['KodeSatker']);
											$subNGOID = $old_subNGOID[1];
