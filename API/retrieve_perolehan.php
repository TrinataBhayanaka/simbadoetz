...
class RETRIEVE_PEROLEHAN extends RETRIEVE{
	...
	public function retrieve_hasil_validasi($parameter)
    {
		...
		$data['kelompok'] 		= $parameter['param']['kelompok_id'];
		$data['lokasi'] 		= $parameter['param']['lokasi_id'];
		$data['sql_where'] 		= TRUE;
		...
		if($dataImplode	!=""){
			$viewTable = 'hasil_validasi_'.$SessionUser['ses_uoperatorid'];
			...
			$sql 	= "SELECT * FROM $viewTable LIMIT $paging, 100 ";echo "sql".$sql; 
			$exec 	= mysql_query($sql) or die(mysql_error());
		...
		if ($dataAsetList !='')
		{
		 ...
		 return array($row,$dataAsetUser,$explode);	
