...
$offset = @$_POST['record'];
    ...
    */
    ...
    function retrieve_searchAset($data,$kodesatker)
    {
        ...
        $sql = mysql_query("SELECT * FROM {$table} WHERE {$setval} AND kodeSatker LIKE '{$kodesatker}%' AND StatusTampil='1' AND Status_Validasi_Barang='1' AND noRegister BETWEEN {$noreg1} AND {$noreg2}");
    ...
    function retrieve_searchAsetDist($data,$kodesatker)
    {
        ...
        $sql = mysql_query("SELECT kodeKelompok,kodeSatker,kodeLokasi,COUNT(*) as kuantitas,MIN(CAST(noRegister AS SIGNED)) as min,MAX(CAST(noRegister AS SIGNED)) as max FROM {$table} WHERE {$setval} AND kodeSatker LIKE '{$kodesatker}%' AND StatusTampil='1' AND (Status_Validasi_Barang IS NULL OR Status_Validasi_Barang = 0) GROUP BY  kodeKelompok ,  kodeLokasi");
        while ($dataAset = mysql_fetch_assoc($sql)){
                    $aset[] = $dataAset;
                }
        // pr($aset);exit;        
    ...
    function retrieve_editkontrak($get){
        ...
        elseif ($get['tbl'] == "G") $get['table'] = 'persediaan';
        ...
        //get from aset
        $sql = "SELECT kodeSatker, TglPerolehan, Satuan, Info, Alamat, kodeLokasi,kodeRuangan, kodeKelompok,SUM(Kuantitas) as Kuantitas, SUM(NilaiPerolehan) as NilaiPerolehan FROM aset WHERE noKontrak = '{$kontrak['noKontrak']}' AND kodeKelompok = '{$get['kdkel']}' AND kodeLokasi = '{$get['kdlok']}' GROUP BY kodeKelompok, kodeLokasi";
        $aset = $this->fetch($sql);
        
        if($get['table'] != 'persediaan'){
            //get kib
