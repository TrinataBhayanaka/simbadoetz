	...
	public function retrieve_usulan_penghapusan_pmd($data,$debug=false)
    {
        ...
        if ($tahun) $filterkontrak .= " AND ast.Tahun = '{$tahun}' ";
                ...
                foreach ($jenisaset as $value) {
                    ...
                    foreach ($resAst as $key => $value) {
                        // $resAst[$key]['value']="1";
                        // $resAst[$key]['asetid']=$value['Aset_ID'];

                        $sqlListTable = array(
                            'table'=>"{$listTable}",
                            'field'=>"{$listTableAlias}.Aset_ID,{$listTableAlias}.kodeSatker,{$listTableAlias}.kodeKelompok,{$listTableAlias}.noRegister,{$listTableAlias}.kondisi,{$listTableAlias}.NilaiPerolehan,{$listTableAlias}.TglPerolehan ",
