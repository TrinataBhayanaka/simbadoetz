...
$res = mysql_query($query) or die(mysql_error());
		...
		$result = $this->query($query1) or die ($this->error());
            if($result)
            {
                ...
                $dataArray1[] = $this->fetch_object($result);
...
$res = mysql_query($query) or die(mysql_error());
		...
		if($tujuan!="")
	{
        ...
        //print_r($query);
        $queryfix="SELECT DISTINCT(a.Aset_ID) ".$query." LIMIT $parameter[paging], 10";
        //pr($queryfix);
  ...
  public function  retrieve_penetapan_pemindahtanganan_eksekusi()
    ...
    public function retrieve_usulan_penghapusan($parameter)
    {
        ...
        echo '</pre>';
                ...
                //print_r($_SESSION);
                $query="SELECT DISTINCT(Aset_ID) FROM Aset ".$_SESSION['ses_retrieve_filter_'.$parameter['menuID'].'_'.$this->UserSes['ses_uid']]." $sql_param ORDER BY Aset_ID ASC LIMIT $parameter[paging], 10";
                ...
                if ($dataArray !='')
                {
                    foreach ($dataArray as $Aset_ID)
                    {
                        ...
                        $query = "SELECT DISTINCT(a.LastSatker_ID), a.NamaAset, a.Aset_ID, a.NomorReg, 
    ...
    */
            ...
            echo '</pre>';
                ...
                if ($dataArray !='')
                {
                    foreach ($dataArray as $Aset_ID)
                    {
                        ...
                        $query = "SELECT a.LastSatker_ID, a.NamaAset, a.Aset_ID, a.NomorReg, 
                                        e.NamaLokasi, f.Uraian, f.Kode,k.NoKontrak,
                                        s.KodeSatker, s.NamaSatker, s.KodeSatker, s.KodeUnit
                                        FROM Aset AS a
                                        INNER JOIN Satker AS s ON a.OrigSatker_ID = s.Satker_ID
                                        INNER JOIN Lokasi AS e  ON a.Lokasi_ID=e.Lokasi_ID
                                        INNER JOIN Kelompok AS f ON a.Kelompok_ID=f.Kelompok_ID
                        ...
                        print_r($query);
                        echo'</pre>';*/
                        //bu query :INNER JOIN  KontrakAset AS c  ON a.Aset_ID = c.Aset_ID
                              //INNER JOIN Kontrak AS b ON b.Kontrak_ID = c.Kontrak_ID
    ...
    {
        $query = "SELECT DISTINCT(aset_list) FROM apl_userasetlist WHERE aset_action = 'PemusnahanUsul'";
        //print_r($query);
        $result = $this->query($query) or die ($this->error());
        ...
        echo '</pre>';
                ...
                if ($dataArray !='')
                {
                    foreach ($dataArray as $Aset_ID)
                    {
                        ...
                        $query = "SELECT a.LastSatker_ID, a.NamaAset, b.Aset_ID, a.NomorReg,e.NamaSatker,e.KodeSatker, 
                                            c.NoKontrak, f.NamaLokasi, g.Kode
                                            FROM UsulanAset AS b
                                            INNER JOIN Aset AS a ON b.Aset_ID=a.Aset_ID
                                            LEFT JOIN  KontrakAset AS d  ON b.Aset_ID = d.Aset_ID
